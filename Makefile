...
# TOP LEVEL TARGETS
# ----------------------------------------
all: install gen move-model uninstall
...
# ---------------------------------------
install: config/env.lock

config/env.lock:
	$(ENV) && pipenv install
	touch config/env.lock
...
move-model:
	...
	cp -r model/schema/* $(PKG_T_SCHEMA)
...
gen-linkml_model: $(patsubst %, $(PKG_T_PYTHON)/%.py, $(SCHEMA_NAMES))
	mkdir -p $(PKG_T_MODEL)
	cp -r model/schema $(PKG_DIR)

$(PKG_T_PYTHON)/%.py: target/python/%.py install
